...
void nrf24_device(uint8_t device_mode, uint8_t reset_state)
{
  ...
  switch (device_mode)
  {
    ...
    case RECEIVER:
      nrf24_mode(POWER_DOWN);
      nrf24_interrupt_mask(DISABLE, ENABLE, ENABLE);                /*Use with IRQ for RX by disable RX mask*/
      nrf24_mode(PRX);
...
/*ENABLE mask to turn off interrupt IRQ */
...
void nrf24_clear_interrupt_mask(void)
{
  nrf24_read(STATUS_ADDRESS, &register_current_value, 1, CLOSE);
  register_new_value |= 0x70;
  nrf24_write(STATUS_ADDRESS, &register_new_value, 1, CLOSE);
}
